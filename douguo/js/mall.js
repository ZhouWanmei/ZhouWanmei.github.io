    ...
    $.ajax({
        ...
        success: function(recommend) {
            ...
            recommend.forEach(function(obj, index){
                htmlStr += `
                    <li>
                        <a href="/douguo/html/goodsDetail.html?goodsId=${obj.id}" target="blank">
                            <img src="/douguo/images/mall/goods-${obj.id}/${obj.img}" alt="图片加载失败...">
                        </a>
                        <p class="describe"><a href="/douguo/html/goodsDetail.html?goodsId=${obj.id}" target="blank" title="${obj.describe}">${obj.describe}</a></p>
                        <p class="price">
